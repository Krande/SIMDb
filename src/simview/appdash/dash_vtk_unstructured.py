...
def ns_export_to_uGrid(n_ns_b64, e_ns_b64, e_data_b64=None):
    ...
    df_nodes = pd.read_csv(io.StringIO(decoded_n.decode('utf-8')), delim_whitespace=True, header=None, skiprows=1,
                           names=['id', 'x', 'y', 'z'])
    ...
    df_elems = pd.read_csv(io.StringIO(decoded_e.decode('utf-8')), skiprows=1, header=None, delim_whitespace=True,
                           engine='python', index_col=None).sort_values(0)
            ...
            dcc.Upload(
                ...
                children=[
                    dbc.Row([
                        ...
                        dbc.Button('Upload Nodes', color='primary')
                    ])
                ]
            ),
            # upload for elems
